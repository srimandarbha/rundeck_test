...
for i in `RD_USER="$RD_TEMP_USER" RD_PASSWORD="$RD_TEMP_PASSWORD" RD_URL=http://localhost:4440/api/19 rd projects list | egrep -v 'Projects|\n' | head -n -2`
do
 ...
 RD_USER="$RD_TEMP_USER" RD_PASSWORD="$RD_TEMP_PASSWORD" RD_URL=http://localhost:4440/api/19 rd projects list | egrep -v 'Projects|\n' | head -n -2 > genfile.txt
done
